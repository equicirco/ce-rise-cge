...
function route_rows(final_registry)
    ...
    for fam in family_definitions()
        push!(rows, [
            ...
            "NEW",
            obs(fam.new_route),
            "virgin_material_tax (indirect via VMTL_EE)",
            "virgin_and_recycled_metal_composite",
            "own_route_price",
            fam.new_route,
        ...
        push!(rows, [
            ...
            "small_metal_parts_plus_eol_input",
            "own_route_price",
            fam.refurbishment_route,
        ...
        push!(rows, [
            ...
            obs("REC_EE"),
            "recycling_support",
            "produces_recycled_metal",
            "recycled_material_price",
            "REC_EE",
            "Family-specific end-of-life flows can be allocated to the common recycling activity that supplies the recycled-metal pool.",
        ])
        push!(rows, [
            fam.family,
            fam.family_label,
...
function quantity_bridge_rows()
    ...
    for row in [
        [
            "VMTL_EE_primary_metal_pool",
            "ALL",
            "VMTL_EE",
            "primary_material_pool",
            "VMTL_EE",
            "CE-RISE-route intermediate demand assembled from BASIC_METALS and METAL_COMPONENTS",
            "kg_metal",
            "P_VMTL_EE",
            "benchmark_unit_value = benchmark_value / benchmark_physical_quantity",
            "Q_t = Q0 * q_t using the primary-material quantity index",
            "If only value outputs are exported, recover quantity as value_t / (benchmark_unit_value * relative_price_t)",
            "template",
            "Primary metal pool used by CE-RISE routes and anchored in explicit upstream sectors.",
        ],
        [
            "RMTL_EE_recycled_metal_pool",
            "ALL",
            "RMTL_EE",
            "secondary_material_pool",
            "RMTL_EE",
            "recycling output and recovered-material evidence",
            "kg_metal",
            "P_RMTL_EE",
            "benchmark_unit_value = benchmark_value / benchmark_physical_quantity",
            "Q_t = Q0 * q_t using the recycled-material quantity index",
            "If only value outputs are exported, recover quantity as value_t / (benchmark_unit_value * relative_price_t)",
            "template",
            "Recycled metal pool supplied by the common recycling activity.",
        ],
        [
...
function coefficient_rows()
    ...
    for row in [
        [
            "ALL_RECOVERY_yield_rmtl_ee",
            "ALL",
            "REC",
            "recovery_yield",
            "yield_rmtl_ee",
            "kg_recycled_metal_per_kg_eol",
            "to_fill",
            "Recovery yield converting recycled end-of-life throughput into secondary metal.",
            "template",
            "Common recycling yield for the CE-RISE material pool.",
        ],
        [
            "ALL_RECYCLED_quality_phi_rmtl_ee",
            "ALL",
            "RMTL",
            "quality_factor",
            "phi_rmtl_ee",
            "effective_metal_per_physical_metal",
            "to_fill",
            "Relative effectiveness of recycled metal compared with primary metal in the CES material composite.",
            "template",
            "Quality or impurity factor for recycled metal in the stylized-consistent material nest.",
        ],
    ]
        push!(rows, row)
    end
...
function validation_pairs(final_registry, route_registry, quantity_template, coefficient_template)
    ...
    return [
        ...
        ("primary_material_pool", "VMTL_EE anchored in BASIC_METALS and METAL_COMPONENTS"),
        ("secondary_material_pool", "RMTL_EE supplied by REC_EE"),
    ]
end
...
function main()
    ...
    family_header = [
        ...
        "primary_material_pool",
        "secondary_material_pool",
        "primary_material_anchor_sectors",
        "service_route_set",
        "eol_route_set",
        "service_nest",
        "notes",
